...
pub fn find_substance_by_name<'a>(name: &str, substances: &'a [Substance]) -> Option<&'a Substance> {
...
pub fn calculate_blood_levels(request: ToleranceRequest) -> Result<ToleranceResponse, String> {
    ...
    let mut substance_intakes: std::collections::HashMap<String, Vec<&SubstanceIntake>> = std::collections::HashMap::new();

    for intake in &request.intakes {
        substance_intakes.entry(intake.substance.clone()).or_insert(Vec::new()).push(intake);
    }
    ...
    for (substance_name, intakes) in substance_intakes {
        ...
        for &time_point in &request.time_points {
            ...
            for &intake in &intakes {
                ...
                let bioavailable_dose = intake.dosage_mg * (substance.bioavailability_percent.unwrap_or(100.0) / 100.0);
            ...
            let safe_total_amount = if total_amount.is_finite() && !total_amount.is_nan() {
                total_amount
            } else {
                0.0
            };
    ...
    Ok(ToleranceResponse {
        blood_levels,
        substances: substances_info,
    })
}