...
export const DieFaceIcon: React.FC<{ sides?: number; className?: string }> = ({ sides = 6, className = 'w-6 h-6' }) => {
  ...
  return (
    <span className={`inline-flex items-center justify-center text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded ${className}`}>
      {label}
    </span>
  );
};
