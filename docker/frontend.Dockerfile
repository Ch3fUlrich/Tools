...
# Build tooling: the `postbuild` step runs scripts/generate-csp.mjs, which hashes
# Next's inline flight scripts into a Content-Security-Policy.
...
# Security headers are generated by the builder so the CSP carries the sha256 of
# the inline scripts in *this* build. A static policy cannot: the hashes change
# every time. See scripts/generate-csp.mjs.
COPY --from=builder --chown=nginx:nginx /workspace/docker/security-headers.generated.conf /usr/share/nginx/security-headers.conf
