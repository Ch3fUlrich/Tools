...
describe('api client full coverage', () => {
  ...
  it('calculateFatLoss success and failure', async () => {
    const good = {
      fat_loss_percentage: 10,
      muscle_loss_percentage: 1,
      is_valid: true,
    };
    ...
    const res = await calculateFatLoss({
      kcal_deficit: 500,
      weight_loss_kg: 1,
    });
    ...
    await expect(
      calculateFatLoss({ kcal_deficit: 0, weight_loss_kg: 0 })
    ).rejects.toThrow(/Failed to calculate fat loss/);
  });
  ...
  it('analyzeN26Data success and failure', async () => {
    ...
    await expect(analyzeN26Data({})).rejects.toThrow(
  ...
  it('register/login/logout success and failure branches', async () => {
    ...
    await expect(
      registerUser({ email: 'a@b.com', password: 'p' })
    ...
    await expect(
      loginUser({ email: 'a@b.com', password: 'p' })
    ).rejects.toThrow(/bad/);
  ...
  it('startOIDCLogin sets window.location.href', () => {
    ...
    startOIDCLogin();
    expect((globalThis as any).window.location.href).toContain(
      '/api/auth/oidc/start'
    );
  });
  ...
  it('rollDice failure branch (already covered success elsewhere)', async () => {
    ...
    await expect(
      rollDice({ die: { type: 'd6' }, count: 1 } as any)
    ).rejects.toThrow(/Roll API error/);
  });
  ...
  it('tolerance endpoints success and failure', async () => {
    ...
    await expect(getToleranceSubstances()).rejects.toThrow(
      /Failed to get substances/
    );

    const resp = {
      blood_levels: [{ time: 't', substance: 's1', amount_mg: 1 }],
    };
    (globalThis as any).fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => resp,
    ...
    await expect(
      calculateTolerance({ intakes: [], time_points: [] })
    ).rejects.toThrow(/Failed to calculate tolerance/);
  });
});
