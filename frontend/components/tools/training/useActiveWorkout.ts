...
export function useActiveWorkout() {
  ...
  function loadSession(full: WorkoutSessionDetail) {
    ...
    for (const s of full.sets) {
      ...
      groupMap[s.exerciseId].sets.push({
        ...
        energyPotentialKcal: s.energyPotentialKcal,
        energyKineticKcal: s.energyKineticKcal,
        energyIsometricKcal: s.energyIsometricKcal,
      });
    }
    setGroups(groupMap);
  }
  ...
  async function handleStart(planId?: string) {
    ...
    try {
      const name = planId
        ? (plans.find((p) => p.id === planId)?.name ?? 'Workout') +
          ' — ' +
  ...
  async function handleRepeatLast() {
    try {
      ...
      if (!last) {
        setError('No completed sessions to repeat');
        return;
      }
  ...
  function addSetToGroup(exerciseId: string) {
    setGroups((prev) => {
      ...
      return {
        ...
        [exerciseId]: {
          ...g,
          sets: [...g.sets, emptyRow(exerciseId, g.sets.length + 1)],
        },
      };
    });
  }
  ...
  const saveSet = useCallback(
    async (exerciseId: string, setIdx: number) => {
      if (!session) return;
      setGroups((prev) => {
        const g = prev[exerciseId];
        if (!g) return prev;
        const sets = g.sets.map((s, i) =>
          i === setIdx ? { ...s, saving: true } : s
        );
        return { ...prev, [exerciseId]: { ...g, sets } };
      });
      try {
        ...
        setGroups((prev) => {
          const pG = prev[exerciseId];
          if (!pG) return prev;
          const sets = pG.sets.map((set, i) =>
            i === setIdx
              ? {
                  ...set,
                  savedId: res.id,
                  saving: false,
                  energyKcal: res.energyKcal,
                  energyPotentialKcal: res.energyPotentialKcal,
                  energyKineticKcal: res.energyKineticKcal,
                  energyIsometricKcal: res.energyIsometricKcal,
                }
              : set
          );
          return { ...prev, [exerciseId]: { ...pG, sets } };
        });
      } catch (e) {
        ...
        setGroups((prev) => {
          const pG = prev[exerciseId];
          if (!pG) return prev;
          const sets = pG.sets.map((s, i) =>
            i === setIdx ? { ...s, saving: false } : s
          );
          return { ...prev, [exerciseId]: { ...pG, sets } };
        });
      }
  ...
  async function handleDeleteSet(exerciseId: string, setIdx: number) {
    ...
    if (s.savedId) {
      try {
        await deleteSet(session.id, s.savedId);
      } catch {
        /* ignore */
      }
    }
