...
function NewMeasurementForm({ onSuccess }: NewMeasurementFormProps) {
    ...
    <CardSection title="New Measurement" gradient="from-orange-500 to-red-600">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <p className="p-4 text-xs text-slate-500 dark:text-slate-400">
          Only body weight is required. Fill in additional measurements to track
          your proportions over time.
        </p>
...
interface PastMeasurementsListProps {
  measurements: BodyMeasurement[];
  onDelete: (id: string) => void;
}

function PastMeasurementsList({
  measurements,
  onDelete,
}: PastMeasurementsListProps) {
  return (
    <CardSection
...
export default function BodyMeasurementsPanel() {
  ...
  return (
    <div className="space-y-8">
      {error && <ErrorAlert error={error} />}
      <NewMeasurementForm onSuccess={loadMeasurements} />
      <PastMeasurementsList
