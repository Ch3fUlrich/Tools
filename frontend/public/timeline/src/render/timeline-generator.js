...
function appendDefinitions(svg, config) {
  ...
  Object.values(config.arrows).forEach((arrow) => {
    ...
    path.setAttribute(
      'd',
      arrow.path || buildArrowPath(arrow.markerWidth, arrow.markerHeight)
    );
...
function appendBackground(svg, config) {
...
function appendDevelopmentStages(svg, config) {
  ...
  const stageBoundaryBottomY =
    stageEditing.boundaryBottomY ??
    (stages[0]?.y ?? 105) + (stages[0]?.height ?? 670);
  ...
  (config.windowLabels || []).forEach((label, index) => {
    ...
    String(label.class || stageLabelSettings.className)
      .split(/\s+/)
      .filter(Boolean)
      .forEach((className) => {
  ...
  for (
    ...
    handleHitbox.setAttribute(
      'height',
      stageBoundaryBottomY - stageBoundaryTopY
    );
...
function appendMainAxis(svg, config) {
  ...
  mainAxisHitbox.setAttribute(
    'stroke-width',
    Math.max(18, Number(config.mainAxis.strokeWidth || 5) + 14)
  );
  mainAxisHitbox.setAttribute('stroke-linecap', 'round');
  svg.appendChild(mainAxisHitbox);
}

function appendAxisLabels(svg, config) {
...
function appendLegacyAxisTicks(svg, config) {
  if (
    Array.isArray(config.humanAxisTicks) &&
    config.axisTicks &&
    config.axisTicks.enabled !== false
  ) {
...
function appendAlignmentNote(svg, config) {
...
function generateTimelineSVG(config) {
  ...
  appendDefinitions(svg, config);
  appendBackground(svg, config);
  appendDevelopmentStages(svg, config);
  appendMainAxis(svg, config);
  appendAxisLabels(svg, config);
  ...
  (config.humanRangeBlocks || []).forEach((block, index) =>
    appendRangeBlockGroup(svg, config, 'human', block, index)
  );
  (config.mouseRangeBlocks || []).forEach((block, index) =>
    appendRangeBlockGroup(svg, config, 'mouse', block, index)
  );

  // Human and mouse nodes
  (config.humanNodes || []).forEach((node, index) =>
    appendNodeGroup(svg, config, 'human', node, index)
  );
  (config.mouseNodes || []).forEach((node, index) =>
    appendNodeGroup(svg, config, 'mouse', node, index)
  );

  appendLegacyAxisTicks(svg, config);
  appendAlignmentNote(svg, config);
...
function initializeTimeline() {
  ...
  if (!config) {
    console.error(
      'TIMELINE_CONFIG not found. Make sure timeline-default-config.json was loaded.'
    );
    return;
  }
