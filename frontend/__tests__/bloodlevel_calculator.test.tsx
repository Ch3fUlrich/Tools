...
describe('BloodLevelCalculator (consolidated)', () => {
  ...
  it('renders and runs main calculation flow', async () => {
    ...
    const container = res.container as HTMLElement;
    ...
    const calcBtn = within(container).getByRole('button', {
      name: /Calculate Blood Levels|calculate blood levels/i,
    });
    await act(async () => {
  ...
  it('covers branches: loaded substances, error handling and interactions', async () => {
    ...
    const getSub = vi
      .spyOn(api, 'getToleranceSubstances')
      .mockResolvedValueOnce([{ id: 's1', name: 'Sub', halfLifeHours: 2 }]);
    const calc = vi
      .spyOn(api, 'calculateTolerance')
    ...
    await waitFor(() =>
      expect(within(c1).getByText('Sub')).toBeInTheDocument()
    );
    ...
    const btn = within(c1).getByRole('button', {
    ...
    await waitFor(() =>
      expect(within(c1).getByText(/Sub Blood Levels/)).toBeInTheDocument()
    );
    ...
    const btn2 = within(c2).getByRole('button', {
      name: /calculate blood levels/i,
    });
    await act(async () => {
    ...
    await waitFor(() =>
      expect(
        within(c2).queryByText(/Calculation failed|Tolerance calc error|boom/i)
      ).toBeTruthy()
    );
  });
  ...
  it('allows adding/removing intakes and covers input handlers', async () => {
    ...
    (vi.spyOn(api, 'calculateTolerance') as any).mockResolvedValueOnce({
      blood_levels: [],
    });
    ...
    const datetime = within(table)
      .getAllByDisplayValue(() => true)
      .find((el) => el.getAttribute('type') === 'datetime-local');
    if (datetime)
      await act(async () => {
        fireEvent.change(datetime, { target: { value: '2025-01-01T12:00' } });
      });

    const intakeTypeSelect = within(table)
      .getAllByRole('combobox')
      .find((s) => s.tagName === 'SELECT');
    if (intakeTypeSelect)
      await act(async () => {
        fireEvent.change(intakeTypeSelect, { target: { value: 'inhaled' } });
      });
    await act(async () => {
