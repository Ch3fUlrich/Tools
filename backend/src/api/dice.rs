...
lazy_static! {
    static ref RATE_LIMIT: Mutex<HashMap<String, (u32, std::time::Instant)>> = Mutex::new(HashMap::new());
}

#[debug_handler]
pub async fn roll(Extension(store): Extension<Option<Arc<AsyncMutex<SessionStore>>>>, headers: axum::http::HeaderMap, Json(payload): Json<JsonValue>) -> impl IntoResponse {
    // Attempt to deserialize into DiceRequest
    let req: dice_logic::DiceRequest = match serde_json::from_value(payload) {
        Ok(r) => r,
        Err(e) => return (axum::http::StatusCode::BAD_REQUEST, axum::Json(serde_json::json!({"error": format!("invalid request: {}", e)}))).into_response(),
    };
    ...
    if let Some(store_arc) = store {
        ...
        if let Some(sid) = SessionStore::parse_sid_from_cookie(cookie_header) {
            ...
            match guard.allow_sliding_window(&key, 60, 20).await {
                Ok(true) => {}
                Ok(false) => return (axum::http::StatusCode::TOO_MANY_REQUESTS, axum::Json(serde_json::json!({"error":"rate limit exceeded"}))).into_response(),
                Err(_) => {
            ...
            if let Ok(ok) = guard.allow_sliding_window(&key, 60, 500) .await {
                if !ok {
                    return (axum::http::StatusCode::TOO_MANY_REQUESTS, axum::Json(serde_json::json!({"error":"rate limit exceeded"}))).into_response();
                }
            }
            limited = true;
        }
    ...
    if !limited {
        ...
        let ip = "unknown".to_string();
        {
            ...
            let entry = map.entry(ip.clone()).or_insert((0, std::time::Instant::now()));
